...
It also stores the reviews behind `/reviews`. Submissions go live instantly;
there is no admin panel. To take one down, add its id to
[`server/src/moderation.mjs`](server/src/moderation.mjs) and redeploy the
server — that file is the only moderation surface, by design.
