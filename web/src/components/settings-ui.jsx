...
// Building blocks for the settings pages. Everything is monochrome on purpose:
// state is carried by weight and contrast, colour is reserved for errors.
...
// Label + description on the left, the control on the right; stacks on phones.
// `wide` puts the control on its own line at every width (inputs, textareas).
...
// Inline field for forms inside a Section (label above the control).
...
// A single raised thumb slides between options.
...
// Button tabs with a sliding underline (the settings pages use links; this is
// for in-page state). `tabs` = [{ id, label, badge? }].
...
export function Modal({ open, title, description, onClose, children, footer, tone = 'default', busy = false }) {
  ...
  if (!open) return null
  // Portaled: the settings column animates with a transform, which would
  // otherwise turn `fixed` into "fixed inside that column".
  return createPortal(
