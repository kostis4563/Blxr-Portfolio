...
// What the table actually stores, for change detection.
...
// Which tab a field error belongs to, so the tab can flag it.
const FIELD_TAB = {
...
function Skeleton() {
  return (
    <div aria-busy="true" className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="flex flex-col gap-5">
        {[220, 260, 140].map((h, i) => (
          <div key={i} style={{ height: h }} className="animate-pulse rounded-xl border border-line bg-surface" />
        ))}
      </div>
...
// blxr.net/u/ + input + availability indicator, one bordered group.
...
// Toggle sections on/off and move them with the arrows. Hidden ones sit at the
// bottom, greyed, so they can be brought back.
...
// Little wireframes of the three layouts.
...
function Editor({ user }) {
  ...
  const [saved, setSaved] = useState(undefined) // undefined loading · null none yet · profile
  ...
  const [previewOpen, setPreviewOpen] = useState(false) // phones only; always open at lg
  ...
  const validate = () => {
    ...
    for (const l of form.links) {
      if (!l.url.trim() && !l.label.trim()) continue // blank row, drop it
