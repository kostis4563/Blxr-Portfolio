...
import { Avatar } from './components/dashboard-topbar'
import {
  Section, Row, Field, Badge, Toggle, Segmented, Select, Modal, CopyButton, ErrorNote, InfoNote,
...
// 0–4, same rule as the login page.
...
// Account prefs live in user_metadata; edits apply instantly and save after a
// short pause so a burst of toggles is one request.
...
function ConnectedAccounts({ user }) {
  ...
  const [confirm, setConfirm] = useState(null) // identity to unlink
  const [error, setError] = useState(null)
  ...
  const connect = async (provider) => {
    ...
    try {
      ...
      // Supabase now owns the page and redirects to the provider.
    } catch (err) {
      setPending(null)
...
function TwoFactorSection() {
  ...
  const [removing, setRemoving] = useState(null) // factor to remove
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState(null)
...
// The sidebar shows Settings as one entry; this strip moves between its pages.
// One underline slides to the active tab instead of each tab drawing its own.
function Tabs({ item }) {
  ...
  return (
    <nav aria-label="Settings sections" className="-mx-4 mb-6 overflow-x-auto border-b border-line px-4 [scrollbar-width:none] sm:mx-0 sm:px-0 [&::-webkit-scrollbar]:hidden">
