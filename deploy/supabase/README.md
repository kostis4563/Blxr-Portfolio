...
Dashboard → Profile → **Import from Discord** lets a member pull their
Discord name, photo, banner, accent colour, avatar decoration, nameplate and
server tag into the profile. No account linking: they paste their Discord
user ID and the server reads the public profile. The flair columns come from
re-running [`profiles.sql`](profiles.sql).

It works out of the box through a public lookup mirror. To read from
Discord's own API instead (recommended — no third party in the path):

1. [discord.com/developers/applications](https://discord.com/developers/applications)
   → **New Application** → name `blxr` (or reuse the one from §6).
2. **Bot** in the left menu → **Reset Token** and copy it (shown once). No
   intents, no permissions and no server invite are needed — the token only
   reads public user objects.
3. GitHub → repo **Settings → Secrets and variables → Actions**: add
   `BLXR_DISCORD_BOT_TOKEN`. The deploy workflow writes it to the server as
   `DISCORD_BOT_TOKEN` on the next push to `main`.

See `server/README.md` → Discord for the route.
